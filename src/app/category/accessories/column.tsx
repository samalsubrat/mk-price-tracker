...
export const columns: ColumnDef<Product>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Link href={`/category/accessories/${slug}`} className="block">
  ...
  },
  {
    ...
    header: ({ column }) => (
      <Button
        ...
        className="h-auto p-0 hover:bg-transparent justify-end w-full text-xs"
      >
        Price
    ...
    cell: ({ row }) => {
      ...
      return <div className="text-right font-medium text-xs sm:text-sm break-words">{price}</div>
    },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="text-right">
          ...
          `}
          >
            <span className="hidden sm:inline">{isInStock ? "In Stock" : "Out of Stock"}</span>
